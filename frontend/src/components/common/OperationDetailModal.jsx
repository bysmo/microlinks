...
export default function OperationDetailModal({ isOpen, onClose, operationId, onSuccess }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [opRes, histRes] = await Promise.all([
        operationApi.findById(operationId),
        operationApi.getHistorique(operationId)
      ]);
      console.log("DEBUG: Operation detail API response:", opRes.data);
      console.log("DEBUG: Operation history API response:", histRes.data);
      setOp(opRes.data);
      setHistorique(histRes.data || []);
    } catch (err) {
      console.error("DEBUG: Failed to load operation data:", err);
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="glass-card w-full max-w-5xl h-[90vh] flex flex-col animate-slide-up border border-white/10 shadow-2xl overflow-hidden">

        {/* Header du Modal */}
        <div className="flex items-center justify-between border-b border-white/5 px-6 py-4 flex-shrink-0 bg-dark-900/80">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg bg-dark-800 border border-dark-700 text-dark-400 hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Corps du Modal */}
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-dark-950/40">

          {loading || !op ? (
            <div className="flex h-64 items-center justify-center">
              <div className="flex flex-col items-center gap-3">
                <RefreshCw className="w-8 h-8 text-primary-400 animate-spin" />
                <span className="text-dark-400 text-sm">Chargement des détails...</span>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Colonne Gauche : Infos Financières & Acteurs */}
                  ...
                  <div className="glass-card p-5 space-y-4 h-full flex flex-col">
                    ...
                    <div className="pt-4 border-t border-white/5 space-y-2.5">
                      ...
                      {op.statut === 'SOUMIS' && isEmetteurCreateur && (
                        ...
                        >
                          <Ban className="w-4 h-4" /> Annuler l'opération
                        </button>
                      )}
                      ...
                      {canValidate && (
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="btn-success btn-sm flex-1 flex items-center justify-center gap-1.5"
                          >
      ...
      {/* ── Sub-Modals pour les actions (Valider, Rejeter, Annuler, Soumettre) ── */}

      {/* ── Modal d'Approbation ── */}
      {showValiderModal && op && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-card w-full max-w-md p-6 space-y-4 animate-slide-up border border-emerald-500/30">
            ...
            <div className="space-y-1.5">
              <label className="text-dark-300 text-xs font-semibold" htmlFor="modal-val-comment">
                Commentaire de validation (optionnel)
              </label>
              <textarea
                id="modal-val-comment"
                value={commentaire}
      ...
      {showRejeterModal && op && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-card w-full max-w-md p-6 space-y-4 border border-red-500/30 animate-slide-up">
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              <h3 className="text-white font-bold text-base flex items-center gap-2">
            ...
            <div className="space-y-1.5">
              <label className="text-dark-300 text-xs font-semibold" htmlFor="modal-rej-motif">
      ...
      {showAnnulerModal && op && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-card w-full max-w-md p-6 space-y-4 border border-amber-500/30 animate-slide-up">
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              <h3 className="text-white font-bold text-base flex items-center gap-2">
            ...
            <div className="space-y-1.5">
              <label className="text-dark-300 text-xs font-semibold" htmlFor="modal-ann-motif">
      ...
      {/* ── Modal de Soumission ── */}
      {showSoumettreModal && op && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-card w-full max-w-md p-6 space-y-4 animate-slide-up border border-primary-500/30">
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              <h3 className="text-white font-bold text-base flex items-center gap-2">
            ...
            <div className="space-y-1.5">
              <label className="text-dark-300 text-xs font-semibold" htmlFor="modal-sub-comment">
                Commentaire de soumission (optionnel)
              </label>
              <textarea
                id="modal-sub-comment"
                value={commentaire}
                onChange={e => setCommentaire(e.target.value)}
