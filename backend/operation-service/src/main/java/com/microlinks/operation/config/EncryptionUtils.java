 ...
 * La clé de chiffrement est configurée par injection Spring.
 */
@Component
@Slf4j
public class EncryptionUtils {
    ...
    private static final String DEFAULT_KEY = "MicroLinksSec#Key256bits!Database";
    ...
    public EncryptionUtils(
            @Value("${database.encryption.key:" + DEFAULT_KEY + "}") String key) {
        byte[] keyBytes = key.getBytes(java.nio.charset.StandardCharsets.UTF_8);
        encryptionKey = Arrays.copyOf(keyBytes, 32);
        log.info("EncryptionUtils initialisé avec chiffrement AES-256-GCM");
    }

    /**
     * Constructeur par défaut (ex: requis hors contexte Spring / JPA).
     */
    public EncryptionUtils() {
     ...
     * @return Chaîne chiffrée encodée en Base64 (contenant IV + données chiffrées).
     */
    ...
    /**
     * Déchiffre une chaîne encodée en Base64 avec AES-256-GCM.
     *
     * @param cipherText Base64 contenant IV + données chiffrées.
    ...
    public static String decrypt(String cipherText) {
        ...
        try {
            ...
            // Fallback si la donnée en base n'est pas chiffrée ou corrompue
